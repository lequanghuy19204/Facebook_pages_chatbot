...
export default function TrainingDocumentsTab() {
  ...
  const handleSyncRag = async () => {
    if (!confirm('Bạn có chắc muốn đồng bộ tất cả tài liệu vào hệ thống RAG?\n\nQuá trình này sẽ:\n1. Xóa toàn bộ dữ liệu RAG cũ\n2. Tạo lại embeddings cho tất cả tài liệu\n\nĐiều này có thể mất vài phút.')) {
      return;
    }

    try {
      setSyncingRag(true);
      toast.info('Đang đồng bộ tài liệu vào RAG...');
      await ApiService.chatbot.syncRagDocuments(token!);
      toast.success('✅ Đồng bộ RAG thành công! Chatbot đã sẵn sàng với dữ liệu mới nhất.');
    } catch (error: any) {
      toast.error('❌ Lỗi đồng bộ RAG: ' + error.message);
    } finally {
      setSyncingRag(false);
    }
  };
  ...
  return (
    <div className="training-doc-tab">
      ...
      <div className="training-doc-header">
        ...
        <div className="training-doc-header-actions">
          <button 
            className="training-doc-btn-secondary" 
            onClick={handleSyncRag}
            disabled={syncingRag || loading}
            title="Đồng bộ tất cả tài liệu vào hệ thống RAG"
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/>
            </svg>
            {syncingRag ? 'Đang đồng bộ...' : 'Đồng bộ RAG'}
          </button>
